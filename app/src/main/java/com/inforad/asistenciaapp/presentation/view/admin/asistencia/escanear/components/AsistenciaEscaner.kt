...
@Composable
fun AsistenciaEscaner(navController: NavHostController, vm: AsistenciaCreateViewModel = hiltViewModel()) {
    ...
    when (val response = vm.createAsistenciaResponse) {
        ...
        is Resource.Failure -> {
            ...
            if (response != null) {
                Toast.makeText(LocalContext.current, "Hubo un error desconocido Cod. L01", Toast.LENGTH_LONG).show()
            }
        }
    }
}
